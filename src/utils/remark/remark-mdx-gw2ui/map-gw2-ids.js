...
import skillsJson from "./mapping/skills.json";
import traitsJson from "./mapping/traits.json";
import itemstatsJson from "./mapping/itemstats.json";
import specializationsJson from "./mapping/specializations.json";
...
const findMatch = ({
  ...
  switch (tag) {
    ...
    case "Specialization": {
      ...
      return specializationsJson.find(
        ({
          id: specializationId,
...
const findMatchWithFallback = (props) => {
  ...
  if (fallback !== undefined) {
    const statsId = itemstatsJson.find(({ name }) => name === affix).ids || {};
